...
const restrictMethodOptions = ref<DictDetail[]>([])

const getOptions = async () => {
  const dictStore = useDictStore()
  const dictOptions = await dictStore.getDictObj([
    'sys_vadmin_gender',
    'vadmin_auth_poi_search_restrict_method'
  ])
  genderOptions.value = dictOptions.sys_vadmin_gender
  restrictMethodOptions.value = dictOptions.vadmin_auth_poi_search_restrict_method
}
...
<template>
  <ContentWrap>
    ...
    >
      ...
      <template #poi_search_restrict_method="{ row }">
        {{ selectDictLabel(restrictMethodOptions, row.poi_search_restrict_method) }}
      </template>
    </Table>
